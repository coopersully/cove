...
describe("E2E: Auth", () => {
	it("register → login → refresh → access protected route", async () => {
		...
		const { status: refreshStatus, body: refreshBody } = await apiRequest(
			"POST",
			"/auth/refresh",
			{ body: { refreshToken } },
		);
...
describe("E2E: User Profile", () => {
	it("register → update profile → other user sees changes via search and profile", async () => {
		...
		const results = searchBody.users as Array<Record<string, unknown>>;
...
describe("E2E: Servers", () => {
	it("create server → comes with #general → list servers → update → delete", async () => {
		...
		const chans = channelsBody.channels as Array<Record<string, unknown>>;
		...
		const serverList = listBody.servers as Array<Record<string, unknown>>;
	...
	it("join public server → message in channel → leave server → cannot message", async () => {
		...
		const msgs = msgsBody.messages as Array<Record<string, unknown>>;
...
describe("E2E: Channels", () => {
	...
	it("non-owner member cannot create/update/delete channels", async () => {
		...
		const { status: createStatus } = await apiRequest(
			"POST",
			`/servers/${serverId}/channels`,
			{
				token: bob.token,
...
describe("E2E: Messages", () => {
	it("send → edit → verify edit → delete → verify gone", async () => {
		...
		const msgs = listBody.messages as Array<Record<string, unknown>>;
	...
	it("message pagination: before cursor and limit", async () => {
		...
		const all = allBody.messages as Array<Record<string, unknown>>;
		...
		const { body: pageBody } = await apiRequest("GET", `/channels/${channelId}/messages`, {
			token: alice.token,
			query: { before: messageIds[2]!, limit: "2" },
		});
		const page = pageBody.messages as Array<Record<string, unknown>>;
