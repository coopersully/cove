...
describe("Friends Routes", () => {
  ...
  describe("POST /friends/requests/:requestId/accept", () => {
    it("accepts a friend request", async () => {
      ...
      const { status, body } = await apiRequest("POST", `/friends/requests/${requestId}/accept`, {
        token: bob.token,
      });
    ...
    it("rejects accept from wrong user", async () => {
      ...
      const { status } = await apiRequest("POST", `/friends/requests/${requestId}/accept`, {
        token: alice.token,
      });
  ...
  describe("GET /friends", () => {
    it("lists friends for both users (catches BigInt comparison bug)", async () => {
      ...
      const aliceFriends = aliceBody.friends as Record<string, unknown>[];
      ...
      const bobFriends = bobBody.friends as Record<string, unknown>[];
